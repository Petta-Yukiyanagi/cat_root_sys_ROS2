...
def generate_test_description():
    ...
    aggregator_node = ExecuteProcess(
        cmd=[
            "/opt/cat_robot/src/ros2_ws/install/diagnostic_aggregator/lib/diagnostic_aggregator/aggregator_node",
            "--ros-args",
            "--params-file",
            "/opt/cat_robot/src/ros2_ws/src/diagnostics/diagnostic_aggregator/test/default.yaml"
            ],
    ...
    add_analyzer = ExecuteProcess(
        cmd=[
            "/opt/cat_robot/src/ros2_ws/install/diagnostic_aggregator/lib/diagnostic_aggregator/add_analyzer",
            "--ros-args",
            "--params-file",
            "/opt/cat_robot/src/ros2_ws/src/diagnostics/diagnostic_aggregator/test/all_analyzers.yaml"
            ],
        name='add_analyzer',
        emulate_tty=True,
        output='screen')
...
class TestAggregator(unittest.TestCase):
    ...
    def test_processes_output(self, proc_output, aggregator_node):
        ...
        proc_output.assertWaitFor(
            expected_output=launch_testing.tools.expected_output_from_file(path="/opt/cat_robot/src/ros2_ws/src/diagnostics/diagnostic_aggregator/test/expected_output/add_all_analyzers"),
